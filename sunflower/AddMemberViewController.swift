...
class AddMemberViewController: UIViewController {
    ...
    @IBOutlet weak var userBedNumberTextField: UITextField!
    ...
    @IBAction func addMemberButtonTapped(sender: AnyObject) {
        ...
        let userBedNumber:String! = userBedNumberTextField.text
        ...
        let myURL = NSURL(string: "http://www.hansberrygarden.org/sunflowermobile/scripts/registerNewMember.php");
        let request = NSMutableURLRequest(URL:myURL!);
        request.HTTPMethod = "POST";
        
        //set HTTP body with parameters from sign up
        //using php key value pattern
        let postString = "userEmail=\(userEmail)&userFirstName=\(userFirstName)&userLastName=\(userLastName)&userAddress=\(userAddress)&userPhoneNumber=\(userPhoneNumber)&userBedNumber=\(userBedNumber)&userPassword=\(userPassword)";
        print(postString)
                ...
                do {
                    ...
                    if let parseJSON:NSDictionary = json {
                        ...
                        if(userID != nil){
                            ...
                            self.displayAlertMessageAndDismissController("Member Added Successfully")
                            //refresh view for next input
